# Generated by Django 3.2.7 on 2021-11-09 10:58

from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Info',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('data', models.CharField(default='', max_length=100)),
                ('Milk', models.CharField(default='', max_length=100)),
                ('PTM', models.CharField(default='', max_length=100)),
                ('Other', models.CharField(default='', max_length=100)),
                ('Case', models.CharField(default='', max_length=100)),
                ('Duy', models.CharField(default='', max_length=100)),
                ('Total', models.CharField(default='', max_length=100)),
            ],
        ),
    ]
